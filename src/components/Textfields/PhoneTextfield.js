...
const PhoneTextfield = ({ label, placeholder, ...props }) => {
  ...
  const handlePhoneNumberChange = (event) => {
    ...
    //   setErrorState({ isError: true, message: "Phone number is too short" })
    //   setPhoneNumber(event.target.value)
  ...
  useEffect(() => {
    if (phoneNumber.length > 0) {
      helpers.setValue(`${countryCode}${phoneNumber}`)
    } else {
      helpers.setValue("")
    }
          ...
          {_countryList.map((country) => {
            return (
              <MenuItem value={country.dial_code}>
                ...
                &nbsp;
                {`+${country.dial_code}`}
                {/* <Emoji symbol={country.flag} label={country.code} /> */}
              </MenuItem>
            )
          })}
        </Select>
